"""V2 Message send / list / stream endpoints.

Uses orchestrator_v2 (inline instructions via Responses API) instead of the
V1 quote_service (OpenAI Prompt Console IDs).  Messages are stored identically
— GET reuses the same DB query logic.
"""
...
from ..services.orchestrator_v2 import orchestrator_v2
from ..services.display_builder import build_error_display

router = APIRouter(
    prefix="/api/v2/conversations/{conversation_id}/messages",
    tags=["messages-v2"],
...
async def send_message(conversation_id: str, body: SendMessageRequest):
    ...
    try:
        msg = await orchestrator_v2.handle_message(conversation_id, body.message)
...
async def send_message_stream(conversation_id: str, body: SendMessageRequest):
    ...
    async def event_generator():
        try:
            async for event in orchestrator_v2.handle_message_stream(
